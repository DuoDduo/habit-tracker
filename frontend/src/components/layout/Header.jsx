      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between py-3.5">
          ...
          <nav className="hidden md:flex items-center bg-gray-100/50 p-1 rounded-xl border border-gray-200/50">
            ...
            <NavItem icon={<BarChart2 size={16} />} label="Analytics" />
          </nav>
          
          {/* 3. Identity & Tools (Right) */}
          <div className="flex items-center gap-2 sm:gap-4">
            <button className="hidden sm:flex p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all">
              <Bell size={20} />
            </button>
            ...
            <div className="flex items-center gap-3 pl-1">
              <div className="hidden lg:flex flex-col text-right">
                <span className="text-xs font-black text-gray-900 leading-none">B.O. James</span>
                <span className="text-[9px] font-mono text-gray-400 uppercase mt-1">Pro Member</span>
              </div>
              <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 p-0.5 shadow-md shadow-blue-100 cursor-pointer hover:rotate-3 transition-transform">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-gray-100 animate-in slide-in-from-top-2 duration-200">
          <nav className="flex flex-col p-4 gap-2">
            <MobileNavItem icon={<LayoutDashboard size={20} />} label="Dashboard" active />
            <MobileNavItem icon={<BarChart2 size={20} />} label="Analytics" />
            <MobileNavItem icon={<Settings size={20} />} label="System Settings" />
            <div className="mt-4 pt-4 border-t border-gray-50">
               <p className="text-[10px] font-mono text-gray-400 uppercase px-3">Session ID: 92134091</p>
            </div>
          </nav>
        </div>
      )}
